...
| Real neural modality validation | UCI EEG Eye State plaintext baseline and sampled N-MNIST plaintext baseline artifacts. Chronological SPD/covariance encoder comparison in `docs/riemannian-eeg-split-note.md`. | Plaintext preprocessing/model evidence only; not medical, encrypted-compute, or deployment evidence. Riemannian flatten did not beat a 1-second Euclidean mean as a classifier on this split. | Add causal SPD recentering on the same split; do not treat raw-accuracy lifts that collapse to the test majority as encoder wins. |
...
| Homomorphic feasibility | TFHE-rs synthetic and EEG real-data runs, OpenFHE BFVrns and CKKS runs with serialized ciphertext-byte and RSS measurements, and native evidence manifest. | Native evidence is local, host-specific, and single-window; not production cryptography or stable performance. | Run multi-window native BFVrns, CKKS, and TFHE-rs sweeps and confirm on the release machine. |
