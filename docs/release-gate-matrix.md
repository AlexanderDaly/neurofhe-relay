...
| `npm run benchmark:tfhe -- --run --input benchmark-artifacts/plaintext-baselines/eeg-eye-state/openfhe-input/eeg-eye-state-bfvrns-contract.json --artifact` | `benchmark-artifacts/comparisons/tfhe-rs-realdata/latest.json` signed-integer run. | Transforms the EEG quantized contract into a signed TFHE-rs contract and runs the native encrypted lane; single-window plaintext-parity evidence only, requires the TFHE-rs build on the release machine, and is not performance or accuracy evidence. |
